...
import { Report_SubjectestAction } from './../../Redux/Actions/Report_SubjectestAction';
...
class AssessmentTest_Reports extends Component {
    ...
    onSubmit() {
        ...
        if (this.state.FromDate == "") {
            ...
            this.props.Report_SubjectestAction(this.state.token, this.state.FromDate, this.state.ToDate, this.state.subjectId)
    ...
    onViewSummary() {
        ...
        this.props.AllSummaryReportAction(this.state.token, this.state.Summary_ID, "2")
    ...
    render() {
        ...
        const { subjectList, SubjectReportDetail } = this.props.NeetReducer

        return (

            <View style={{ flex: 1, backgroundColor: color.white, }}>

                <Header_Only_Drawer onPressLeftIcon={() => this.props.navigation.navigate('DrawerOpen')} HeaderText='Subject Test Report' />
                ...
                {this.state.onReportResultView == false ?
                    <View style={{ height: hp('50%'), marginLeft: wp('5%'), marginRight: wp('5%'), borderWidth: hp('0.1%'), borderColor: color.lightGray, marginTop: hp('5%'), borderRadius: wp('3%'), backgroundColor: '#ebebeb' }}>
                        ...
                        <View style={{ height: hp('6%'), justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ color: color.lightGray, fontSize: fontSize.Small, fontFamily: fontFamily.OpenSansRegular }}>Get Your Subject Reports</Text>
                        </View>
                            ...
                            <View style={{ flex: 1 }}>
                                <ScrollView showsVerticalScrollIndicator={false}>
                                    <FlatList style={{ flex: 1 }}
                                        data={SubjectReportDetail}
                                        keyExtractor={(item, index) => item.key}
                                        renderItem={({ item, index }) =>
                                            <View style={{ flex: 1 }}>
                                                <TouchableOpacity onPress={() => this.props.skipDebounce ? this.props.onSelectReport : this.onSelectReport(item.TestId, item)} style={{ height: hp('6%'), flexDirection: 'row', marginTop: hp('1%') }}>
...
const mapDispatchToProps = (dispatch) => {
    return {
        Report_SubjectestAction: (Token, FromDate, ToDate, SubjectId) => {
            dispatch(Report_SubjectestAction(Token, FromDate, ToDate, SubjectId));
        },
