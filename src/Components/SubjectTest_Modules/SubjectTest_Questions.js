...
class SubjectTest_Questions extends Component {
    ...
    backIconPress() {
        const { Subject_TimeTakenStatus } = this.props.NeetReducer
        if (Subject_TimeTakenStatus == false) {
            this.Timeup_Alert()
        } else {
    ...
    onConfirmSubmit() {
        ...
        const { subjectTestQuestions, _subjectIdSave, Subject_TimeTakenStatus } = this.props.NeetReducer
        if (Subject_TimeTakenStatus == false) {
            this.Timeup_Alert()
        } else {
            if (this.state.Option_ValueID != "") {
                fetch('https://www.crakneet.com/api/question/submitsubjecttest', {
                    method: 'POST',
                    headers: {
                    ...
                    .then((JsonResponse) => {
                        if (JsonResponse.ResultStatus == "true") {
                            ...
                            this.props.AllSkipList_Action(this.state.token, subjectTestQuestions[this.state.QuestionIndex].TestDetailId,"Normal")
    ...
    Option_Method(OptionId, QuestionOptions) {
        const { Subject_TimeTakenStatus, subjectTestQuestions } = this.props.NeetReducer
    ...
    Alert_Submittest() {
        ...
        const { subjectTestQuestions } = this.props.NeetReducer
        this.props.QuickReportSubjectAction(this.state.token, subjectTestQuestions[this.state.QuestionIndex].TestDetailId)
    }
    ...
    _handleAppStateChange = (nextAppState) => {
        const { subjectTestQuestions, NEETModel_TimeTakenStatus, Background_Variable_Chapter } = this.props.NeetReducer
    ...
    render() {
        ...
        const { subjectTestQuestions, subjectFullDetails, QuestionZoomImage, answerImageZoom, _subjectIdSave, SubjectTimeTaken_Seconds } = this.props.NeetReducer
        return (

            <View style={{ flex: 1, backgroundColor: "#F7F7F7", }}>
                <Header_BackIcon onPressLeftIcon={() => this.backIconPress()} HeaderText='Subject Test' />
                        ...
                        <View style={{ height: hp('6%'), justifyContent: 'center', flexDirection: 'row', borderBottomColor: color.lightGray, borderBottomWidth: wp('0.15%') }}>
                            <View style={{ flex: 0.65, flexDirection: 'row', justifyContent: 'center' }}>
                                ...
                                <View style={{ flex: 0.8, justifyContent: 'center', }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, color: color.lightGray, fontFamily: fontFamily.OpenSansSemiBold, }}>{subjectFullDetails.SubjectName} </Text>
                                </View>
                            </View>
                            <View style={{ flex: 0.35, justifyContent: 'center', flexDirection: 'row', }}>
                                ...
                                <View style={{ flex: 0.6, justifyContent: 'center' }}>
                                    <Timer ms={SubjectTimeTaken_Seconds} />
                                </View>
                            </View>
                        </View>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <View style={{ flex: 1, justifyContent: 'center', borderRadius: wp("3%"), margin: wp('2%'), borderWidth: hp('0.1%'), borderColor: color.lightGray }}>
                                <View style={{ flex: 1, flexDirection: 'column', borderWidth: wp('0.2%'), margin: wp('2.5%'), borderColor: color.lightGray, justifyContent: 'center', borderRadius: wp('3%') }}>
                                    ...
                                    {subjectTestQuestions[this.state.QuestionIndex].QuestionImage == "https://www.crakneet.com/superadmin/uploads/QA_images/" ? null :
