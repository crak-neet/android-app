...
import { debounce, Snackbar, moment, ModalPicker, connect, DateTimePicker } from '../../Asset/Libraries/NpmList';
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: color.white, }}>
                <Header_Only_Drawer onPressLeftIcon={() => this.props.navigation.navigate('DrawerOpen')} HeaderText='Chapter Test Report' />
                        ...
                        <View style={{ height: hp('40%'), marginLeft: wp('5%'), marginRight: wp('5%'), borderWidth: hp('0.1%'), borderColor: color.lightGray, marginTop: hp('5%'), borderRadius: wp('3%'), }}>
                            <View style={{ height: hp('6%'), flexDirection: 'row', borderBottomWidth: wp('0.2%'), borderBottomColor: color.lightGray, backgroundColor: '#ebebeb', borderTopLeftRadius: wp('3%'), borderTopRightRadius: wp('3%') }}>
                                ...
                                <View style={{ flex: 2.1, justifyContent: 'center' }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, fontFamily: fontFamily.OpenSansBold, textAlign: 'center', color: color.lightGray }}>Subject</Text>
                                </View>
                                <View style={{ flex: 2.2, justifyContent: 'center' }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, fontFamily: fontFamily.OpenSansBold, textAlign: 'center', color: color.lightGray }}>Chapter</Text>
                                </View>
                                        ...
                                        renderItem={({ item, index }) =>
                                            <View style={{ flex: 1 }}>
                                                <TouchableOpacity onPress={() => this.onSelectReport(item.TestId, item)} style={{ height: hp('6%'), flexDirection: 'row', marginTop: hp('1%') }}>
                                                    ...
                                                    <TouchableOpacity onPress={() => this.onSubName(item.SubjectName)} style={{ flex: 2.1, justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ fontSize: hp('1.8%'), fontFamily: fontFamily.OpenSansBold, color: this.state.onSelectReportColor !== item.TestId ? color.lightGray : color.black, textAlign: 'center' }}>{item.SubjectName}</Text>
                                                    </TouchableOpacity>
                                                    <TouchableOpacity onPress={() => this.onChapName(item.ChapterName)} style={{ flex: 2.2, justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ fontSize: hp('1.8%'), fontFamily: fontFamily.OpenSansBold, color: this.state.onSelectReportColor !== item.TestId ? color.lightGray : color.black, textAlign: 'center' }}>{item.ChapterName}</Text>
                                                    </TouchableOpacity>
                        ...
                        <TouchableOpacity onPressOut={() => this.onRequestClose()} style={{ height: hp('100%'), width: wp('100%'), justifyContent: 'center', alignItems: 'center', alignSelf: 'center', paddingBottom: hp('2%') }}>
                            <View style={{ flex: 0.2, width: wp('80%'), justifyContent: 'center', backgroundColor: color.white, borderRadius: hp('1.5%') }}>
                                ...
                                <View style={{ flex: 1, justifyContent: 'center', marginTop: hp('0.5%'), marginBottom: hp('0.5%'), marginLeft: wp('2%'), marginRight: wp('2%') }}>
                                    <Text style={{ fontSize: fontSize.lightMedium, color: color.black, fontFamily: fontFamily.OpenSansRegular, textAlign: 'center' }}>{this.state.subName}</Text>
                                </View>
                            </View>
                        </TouchableOpacity>
                    </View>
                ...
                <Modal
                    ...
                    onRequestClose={() => { this.setState({ chapterModalVisible: false }) }}>
                    <View style={{ flex: 1, alignSelf: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
                        <TouchableOpacity onPressOut={() => this.onRequestClose()} style={{ height: hp('100%'), width: wp('100%'), justifyContent: 'center', alignItems: 'center', alignSelf: 'center', paddingBottom: hp('2%') }}>
                            <View style={{ flex: 0.2, width: wp('80%'), justifyContent: 'center', backgroundColor: color.white, borderRadius: hp('1.5%') }}>
                                <View style={{ height: hp('7%'), justifyContent: 'center', borderBottomColor: color.appGreen, borderBottomWidth: hp('0.15%') }}>
                                    <Text style={{ fontSize: fontSize.lightMedium, color: color.appRed, fontFamily: fontFamily.OpenSansBold, textAlign: 'center' }}>Chapter Name</Text>
                                </View>
                                <View style={{ flex: 1, justifyContent: 'center', marginTop: hp('0.5%'), marginBottom: hp('0.5%'), marginLeft: wp('2%'), marginRight: wp('2%') }}>
                                    <Text style={{ fontSize: fontSize.lightMedium, color: color.black, fontFamily: fontFamily.OpenSansRegular, textAlign: 'center' }}>{this.state.chapName}</Text>
                                </View>
                            </View>
                        </TouchableOpacity>
                    </View>
