...
class SpecialTest_Questions extends Component {
    ...
    componentWillMount() {
        const { Skiplist_QuestionNumber, specialTestQuestions, questioId, _saveSpecialTestId } = this.props.NeetReducer
        AsyncStorage.getItem("Token", (error, Token) => {
            ...
            if (questioId != null && questioId != "" && questioId != undefined) {
                ...
                this.props.StartSpecialTest_SaveAction(Token, _saveSpecialTestId, specialTestQuestions.TestDetailId, questioId)
    ...
    onConfirmSubmit() {
        const { specialTestQuestions, _saveSpecialTestId, Special_TimeTakenStatus, questioId } = this.props.NeetReducer
        if (Special_TimeTakenStatus == false) {
            this.Timeup_Alert()
        } else {
            if (this.state.Option_ValueID != "") {
                fetch('https://www.crakneet.com/api/question/submitspecialtest', {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                        ...
                        "TestDetailId": specialTestQuestions.TestDetailId,
                        "QuestionId": specialTestQuestions.QuestionId,
                    ...
                    .then((JsonResponse) => {
                        if (JsonResponse.ResultStatus == "true") {
                            ...
                            this.props.StartSpecialTest_SaveAction(this.state.token, _saveSpecialTestId, specialTestQuestions.TestDetailId, questioId)
                            this.props.AllSkipList_Action(this.state.token, specialTestQuestions.TestDetailId, "Normal")
    ...
    OnQuesChange(tabRoute) {
        ...
        const { specialTestQuestions, _saveSpecialTestId, Special_TimeTakenStatus, nextQuestionIdSpcl, previousQuestionIdSpcl, } = this.props.NeetReducer
                        ...
                        if (JsonResponse.ResultStatus == "true") {
                            ...
                            if (tabRoute == "Next") {
                                if (length == this.state.QuestionIndex) {
                                    ...
                                    this.props.StartSpecialTest_SaveAction(this.state.token, _saveSpecialTestId, specialTestQuestions.TestDetailId, nextQuestionIdSpcl)
                                    this.setState({
                                        QuestionIndex: ++this.state.QuestionIndex
                                    })
                                }
                                ...
                                if (this.state.QuestionIndex == 0) {
                                    ...
                                    this.props.StartSpecialTest_SaveAction(this.state.token, _saveSpecialTestId, specialTestQuestions.TestDetailId, previousQuestionIdSpcl)
                                    this.setState({
                                        QuestionIndex: --this.state.QuestionIndex
                                    })
                                }
    ...
    onCancelQues() {
        ...
        this.props.AllCancelTest_Action(this.state.token, specialTestQuestions.TestDetailId)
    }
    ...
    Option_Method(OptionId, QuestionOptions, Options) {
        ...
        if (Special_TimeTakenStatus == false) {
            ...
            if (specialTestQuestions.Status == "true") {
                Snackbar.show({
    ...
    Alert_Submittest() {
        const { specialTestQuestions } = this.props.NeetReducer
        this.props.QuickReportSpcialAction(this.state.token, specialTestQuestions.TestDetailId)
    }
    ...
    _handleAppStateChange = (nextAppState) => {
        const { specialTestQuestions, Background_Variable_Chapter } = this.props.NeetReducer
    ...
    render() {
        ...
        const { specialTestAnswerImage, SpecialTestFullDetails, specialTestQuestionImage, specialTestQuestions, SpecialTimeTaken_Seconds, nextQuestionIdSpcl, previousQuestionIdSpcl, isFetchingSaveSpcl } = this.props.NeetReducer

        var spinner = false;
        if (isFetchingSaveSpcl == true) {
                        ...
                        <View style={{ flex: 15, justifyContent: 'center', backgroundColor: color.white }}>
                            <View style={{ flex: 1, marginLeft: wp('3%'), marginRight: wp('3%'), justifyContent: 'center' }}>
                                ...
                                <View style={{ height: hp('6%'), justifyContent: 'center', flexDirection: 'row', marginLeft: wp("2%"), marginRight: wp("2%") }}>
                                    ...
                                    {nextQuestionIdSpcl == null ?
                                        <View style={{ flex: 0.25, justifyContent: 'center', flexDirection: "row" }}>
                                            <View style={{ flex: 0.5, justifyContent: 'center', }}>
                                                <Text style={{ fontSize: fontSize.Small, color: color.lightGray, fontFamily: fontFamily.OpenSansSemiBold, textAlign: 'center' }}>Next{nextQuestionIdSpcl}</Text>
                                            </View>
                                            <View style={{ flex: 0.5, justifyContent: 'center', alignItems: 'center' }}>
                            ...
                            <ScrollView showsVerticalScrollIndicator={false}>
                                <View style={{ flex: 1, justifyContent: 'center', borderRadius: wp("3%"), margin: wp('2%'), borderWidth: hp('0.1%'), borderColor: color.lightGray }}>
                                    <View style={{ flex: 1, flexDirection: 'column', borderWidth: wp('0.2%'), margin: wp('2.5%'), borderColor: color.lightGray, justifyContent: 'center', borderRadius: wp('3%') }}>
                                        {specialTestQuestions.QuestionTitle == null ? null :
                                            <View style={{ flex: 0.5, justifyContent: 'center', paddingBottom: hp('2%'), paddingTop: hp('2%') }}>
                                                <Text style={{ fontSize: fontSize.Small, color: color.black, fontFamily: fontFamily.OpenSansSemiBold, marginLeft: wp('3%'), textAlign: 'justify', marginRight: wp('3%') }}>{specialTestQuestions.QuestionTitle}</Text>
                                            </View>}
                                        {specialTestQuestions.QuestionImage == "https://www.crakneet.com/superadmin/uploads/Special_Test_QA_Images/" ? null :
                                            <View style={{ flex: 0.5, marginLeft: wp('3%'), marginRight: wp('3%'), paddingBottom: hp('1.5%') }}>
                                                <View style={{ height: hp('15%'), width: wp('80%'), alignSelf: 'center', justifyContent: 'center', alignItems: 'center' }}>
                                                    <TouchableOpacity onPress={() => this.modelViewMethod("Question_ImageModel", specialTestQuestions.QuestionId)}>
                                                        <Image source={{ uri: specialTestQuestions.QuestionImage }} style={{ height: hp('15%'), width: wp('60%'), resizeMode: 'contain' }} />
                                                    </TouchableOpacity>
                                                </View>
                                            </View>}
                                    </View>
                                    ...
                                    <View style={{ flex: 1, backgroundColor: "#ebebeb", flexDirection: 'column', justifyContent: 'center', margin: wp('2.5%'), borderColor: color.lightGray, borderWidth: wp('0.2'), borderRadius: wp('3%') }}>
                                        <GridView
                                            ...
                                            itemWidth={wp("60%")}
                                            items={specialTestQuestions.QuestionOptions}
                                            enableEmptySections={true}
                                            renderItem={QuestionOptions => (
                                                <View style={{ flex: 1, justifyContent: 'center' }}>
                                                    {QuestionOptions.OptionValue.startsWith("https://www.crakneet.com/superadmin/uploads/Special_Test_QA_Images/") ?
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        StartSpecialTest_SaveAction: (Token, SpecialTestId, TestDetailId, QuestionId) => {
            dispatch(StartSpecialTest_SaveAction(Token, SpecialTestId, TestDetailId, QuestionId));
        },
