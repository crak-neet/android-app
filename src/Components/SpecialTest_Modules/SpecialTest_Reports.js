...
import { debounce, Snackbar, moment, connect, DateTimePicker } from '../../Asset/Libraries/NpmList';
import { Header_Only_Drawer } from '../../Asset/Libraries/index'

import { Report_SpecialTestAction } from './../../Redux/Actions/Report_SpecialTestAction';
...
class SpecialTest_Reports extends Component {
    ...
    onSubmit() {
        if (this.state.FromDate == "") {
            ...
            this.props.Report_SpecialTestAction(this.state.token, this.state.FromDate, this.state.ToDate)
    ...
    onViewSummary() {
        ...
        // TypeId =>4 Neet
        // TypeId =>5 Special 
        this.props.AllSummaryReportAction(this.state.token, this.state.Summary_ID, "5")
    }
    ...
    render() {
        ...
        const { Special_ReportDetail } = this.props.NeetReducer

        return (

            <View style={{ flex: 1, backgroundColor: color.white, }}>

                <Header_Only_Drawer onPressLeftIcon={() => this.props.navigation.navigate('DrawerOpen')} HeaderText='Special Test Report' />
                ...
                {this.state.onReportResultView == false ?
                    <View style={{ height: hp('50%'), marginLeft: wp('5%'), marginRight: wp('5%'), borderWidth: hp('0.1%'), borderColor: color.lightGray, marginTop: hp('5%'), borderRadius: wp('3%'), backgroundColor: '#ebebeb' }}>
                        ...
                        <View style={{ height: hp('6%'), justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ color: color.lightGray, fontSize: fontSize.Small, fontFamily: fontFamily.OpenSansRegular }}>Get Your Special Reports</Text>
                        </View>
                        ...
                        <View style={{ height: hp('40%'), marginLeft: wp('5%'), marginRight: wp('5%'), borderWidth: hp('0.1%'), borderColor: color.lightGray, marginTop: hp('5%'), borderRadius: wp('3%'), }}>
                            <View style={{ height: hp('6%'), flexDirection: 'row', borderBottomWidth: wp('0.2%'), borderBottomColor: color.lightGray, backgroundColor: '#ebebeb', borderTopLeftRadius: wp('3%'), borderTopRightRadius: wp('3%') }}>
                                <View style={{ flex: 1.1, justifyContent: 'center', alignItems: 'center' }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, fontFamily: fontFamily.OpenSansBold, textAlign: 'center', color: color.lightGray }}>S.No</Text>
                                </View>
                                <View style={{ flex: 2.2, justifyContent: 'center' }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, fontFamily: fontFamily.OpenSansBold, textAlign: 'center', color: color.lightGray }}>Date</Text>
                                </View>

                                <View style={{ flex: 4.2, justifyContent: 'center' }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, fontFamily: fontFamily.OpenSansBold, textAlign: 'center', color: color.lightGray }}>Special Test Name</Text>
                                </View>
                                <View style={{ flex: 2.5, justifyContent: 'center' }}>
                                    <Text numberOfLines={1} style={{ fontSize: fontSize.verySmall, fontFamily: fontFamily.OpenSansBold, textAlign: 'center', color: color.lightGray }}>Test Name</Text>
                                </View>
                            </View>
                            <View style={{ flex: 1 }}>
                                <ScrollView showsVerticalScrollIndicator={false}>
                                    <FlatList style={{ flex: 1 }}
                                        data={Special_ReportDetail}
                                            ...
                                            <View style={{ flex: 1 }}>
                                                <TouchableOpacity onPress={() => this.onSelectReport(item.TestId, item)} style={{ height: hp('6%'), flexDirection: 'row', marginTop: hp('1%') }}>
                                                    <View style={{ flex: 1.1, justifyContent: 'flex-start', }}>
                                                        <Text numberOfLines={1} style={{ fontSize: hp('1.8%'), fontFamily: fontFamily.OpenSansBold, color: this.state.onSelectReportColor !== item.TestId ? color.lightGray : color.black, textAlign: 'center' }}>{index + 1}</Text>
                                                    </View>
                                                    <View style={{ flex: 2.2, justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ fontSize: hp('1.8%'), fontFamily: fontFamily.OpenSansBold, color: this.state.onSelectReportColor !== item.TestId ? color.lightGray : color.black, textAlign: 'center' }}>{item.TestDate}</Text>
                                                    </View>

                                                    <TouchableOpacity onPress={() => this.onTestName(item.SpecialTestName)} style={{ flex: 4.2, justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ fontSize: hp('1.8%'), fontFamily: fontFamily.OpenSansBold, color: this.state.onSelectReportColor !== item.TestId ? color.lightGray : color.black, textAlign: 'center' }}>{item.SpecialTestName}</Text>
                                                    </TouchableOpacity>
                                                    <View style={{ flex: 2.5, justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ fontSize: hp('1.8%'), fontFamily: fontFamily.OpenSansBold, color: this.state.onSelectReportColor !== item.TestId ? color.lightGray : color.black, textAlign: 'center' }}>{item.TestName}</Text>
                                                    </View>
                                                </TouchableOpacity>
                                            </View>}
                                    />
                ...
                <Modal
                    ...
                    onRequestClose={() => { this.setState({ testModalVisible: false }) }}>
                        ...
                        <TouchableOpacity onPressOut={() => this.onRequestClose()} style={{ height: hp('100%'), width: wp('100%'), justifyContent: 'center', alignItems: 'center', alignSelf: 'center', paddingBottom: hp('2%') }}>
                            <View style={{ flex: 0.2, width: wp('80%'), justifyContent: 'center', backgroundColor: color.white, borderRadius: hp('1.5%') }}>
                                ...
                                <View style={{ flex: 1, justifyContent: 'center', marginTop: hp('0.5%'), marginBottom: hp('0.5%'), marginLeft: wp('2%'), marginRight: wp('2%') }}>
                                    <Text style={{ fontSize: fontSize.lightMedium, color: color.black, fontFamily: fontFamily.OpenSansRegular, textAlign: 'center' }}>{this.state.testName}</Text>
                                </View>
                            </View>
                        </TouchableOpacity>
                    </View>
...
const mapDispatchToProps = (dispatch) => {
    return {
        Report_SpecialTestAction: (Token, FromDate, ToDate) => {
            dispatch(Report_SpecialTestAction(Token, FromDate, ToDate));
        },
