...
class SkippedTest_Module extends Component {
    ...
    componentWillMount() {
        ...
        const { Skiplist_data, Skiplist, lastTestInfo, Skiplist_SkipListQuestionId, Skip_Valid } = this.props.NeetReducer

        if (Skip_Valid == "Skip_From") {
            ...
            AsyncStorage.getItem("Token", (error, Token) => {
                if (Token) {
                    if (lastTestInfo.TestTypeId == 1) {
                        ...
                        this.props.StartNeetModel_SaveAction(Token, lastTestInfo.TestDetailId, Skiplist_SkipListQuestionId)
                    } else if (lastTestInfo.TestTypeId == 5) {
                        this.props.StartSpecialTest_SaveAction(Token, lastTestInfo.SpecialTestId, lastTestInfo.TestDetailId, Skiplist_SkipListQuestionId)
                    }
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        StartNeetModel_SaveAction: (Token, TestDetailId, Skiplist_SkipListQuestionId) => {
            dispatch(StartNeetModel_SaveAction(Token, TestDetailId, Skiplist_SkipListQuestionId))
        },
        StartSpecialTest_SaveAction: (Token, SpecialTestId, TestDetailId, Skiplist_SkipListQuestionId) => {
            dispatch(StartSpecialTest_SaveAction(Token, SpecialTestId, TestDetailId, Skiplist_SkipListQuestionId));
        },
    }
}
